...
本書では、リポジトリ全体の設計思想・主要機能・構成要素を俯瞰し、Python 版ドキュメントにおける考え方との対応関係を整理します。v0.2.0 では tRPC を廃止し、Next.js 標準の Server Actions による型安全な API 通信へ移行しました。現在の依存構成は Next.js 16.2.9、React 19.2.7、Prisma 7.8.0、TypeScript 6.0.3、Node.js 22.20 を基準としています。
...
| カテゴリ | 採用技術 | 役割 | リンク |
| --- | --- | --- | --- |
| ランタイム | Node.js 22.20 | ローカル開発・Docker 実行環境 | [Node.js](https://nodejs.org/) |
| Web フレームワーク | Next.js 16.2.9 App Router | サーバーサイドレンダリングと API ルーティング | [Next.js](https://nextjs.org/) |
| 言語 | TypeScript 6.0.3 / React 19.2.7 | 型安全と最新 RSC 互換 UI | [TypeScript](https://www.typescriptlang.org/) |
| ORM / DB | Prisma 7.8.0 + PostgreSQL 16 (adapter-pg) | スキーマ定義・マイグレーション・DB アクセス | [Prisma](https://www.prisma.io/) |
| 認証 | NextAuth.js 4.24.14 (Prisma Adapter) | セッション・RBAC 補助 | [NextAuth.js](https://next-auth.js.org/) |
...
| キュー / ジョブ | BullMQ 5.78 + Redis 7 | 非同期ジョブ処理・メール送信 | [BullMQ](https://docs.bullmq.io/) |
| ロギング | Pino 10.3 | JSON ログ出力・構造化ログ | [Pino](https://getpino.io/) |
| レート制御 | rate-limiter-flexible 11.2 + Redis | API プロキシでのリクエスト制御 | [rate-limiter-flexible](https://github.com/animir/node-rate-limiter-flexible) |
| メール | Nodemailer 9 | SMTP 経由の通知送信 | [Nodemailer](https://nodemailer.com/) |
...
├─ prisma.config.ts     # Prisma 7 CLI 設定と DATABASE_URL の読み込み
├─ src/
...
├─ Dockerfile           # Node.js 22.20 ベースのマルチステージビルド
├─ AGENTS.md            # コーディングエージェント向け Next.js 開発ルール
├─ CLAUDE.md            # AGENTS.md 参照設定
...
* Todo 操作など認証が必要な Action では `getServerSession(authOptions)` を呼び出し、セッションとユーザー ID を検証します。ユーザー登録やパスワードリセット要求は未認証ユーザー向けの公開 Action です。
* 個別 Action ファイルの責務:
...
* **Zod によるバリデーション**: 外部入力を受け取る Action は Zod スキーマで検証し、型安全性を確保します。
* **エラーハンドリング**: `try-catch` でエラーを捕捉し、`{ success: boolean, error?: string, data?: T }` 形式の結果を返します。
* `auth.ts` は Prisma の一意制約違反 (`P2002`) と DB 接続障害 (`P1000` から `P1003`、`ECONNREFUSED`) を判別します。内部エラーは Pino へ記録し、クライアントには内部構造を露出しないメッセージを返します。
...
* Prisma 7 では接続 URL を `schema.prisma` に記述せず、リポジトリルートの `prisma.config.ts` で管理します。`dotenv/config` で `.env` を明示的に読み込み、`env('DATABASE_URL')` を Prisma CLI に渡します。
* Client generator は既存コードとの互換性を維持するため `prisma-client-js` を使用し、アプリケーションコードでは引き続き `@prisma/client` から import します。
* マイグレーションは `pnpm prisma migrate dev` で生成・適用できます。Docker 実行時は app / worker の起動前に `pnpm prisma migrate deploy` を自動実行します。
* Prisma 7 の実行要件は Node.js 20.19 以上です。本プロジェクトの Docker イメージは Node.js 22.20 に固定しています。
...
* `.env.example` に必須/任意の設定が整理されています。
  * `DATABASE_URL` — PostgreSQL への接続。アプリ本体と `prisma.config.ts` の双方から参照します。
  * `NEXTAUTH_URL` / `NEXTAUTH_SECRET` — セッション生成に必須。
  * `REDIS_URL` — レートリミットと BullMQ を有効化する場合に設定。
  * `SMTP_*` — メール送信用トランスポート。
* `next.config.ts` は `typedRoutes: true` を明示し、`Link` やルート指定の型安全性を有効化しています。
* `tsconfig.json` では strict mode を強制し、`moduleResolution: "bundler"`、`target: "es2022"`、`jsx: "react-jsx"` を採用しています。Next.js 用の型定義プラグインも組み込んでいます。
* `package.json` の `engines.node` は Prisma 7 の最低要件に合わせて `>=20.19.0` としています。標準の Docker 実行環境は Node.js 22.20 です。

### 依存関係のセキュリティ設定

* `pnpm.overrides` で監査済みの推移依存バージョンを固定しています。対象は `diff`、`effect`、`@hono/node-server`、`ioredis`、`postcss`、`uuid` です。
* NextAuth.js 4 系の peer dependency 宣言より新しい Nodemailer 9 を利用するため、`pnpm.peerDependencyRules.allowedVersions` で許容バージョンを明示しています。
* 依存更新時は `pnpm audit`、型チェック、Prisma Client 生成、Next.js 本番ビルドを一式で確認してください。
...
1. Node.js の確認
   ...
   node --version
   ```
   Node.js 20.19 以上が必要です。Docker と同じ条件にそろえる場合は Node.js 22.20 を使用します。
2. 依存関係のインストール
   ```bash
   pnpm install
   ```
3. 環境変数の準備
   ...
   cp .env.example .env
   ```
   Windows PowerShell では `Copy-Item .env.example .env` を使用します。ローカルから Compose の PostgreSQL へ接続する場合、`DATABASE_URL` のホストは `localhost:5432` にします。
4. Prisma Client の生成とマイグレーション
   ```bash
   pnpm prisma generate
   pnpm prisma migrate dev
   ```
   新しいマイグレーションを作成する場合のみ `--name <migration-name>` を追加します。
5. 開発サーバー起動 (Turbopack)
   ```bash
   pnpm dev
   ```
   > 開発は Turbopack (`next dev --turbo`) を利用します。本番ビルドは `pnpm build`（`next build`）で、Next.js 16 のデフォルト設定に従います。
6. バックグラウンドワーカーの起動 (Redis 利用時)
   ```bash
   pnpm worker
   ```
7. 必要に応じて `.env` を編集し、`REDIS_URL` や SMTP 設定を追加します。
...
* Redis/メールまで含めて動作確認する場合:
  ...
  ```
* Dockerfile は Node.js 22.20 を固定したマルチステージ構成で、依存解決 → Prisma Client 生成 → `next build`（Turbopack）→ ランタイムの順にイメージを構築します。
* ランタイムイメージには `prisma.config.ts`、`prisma/`、Prisma CLIを含む依存関係をコピーし、コンテナ起動時の `prisma migrate deploy` を可能にしています。
* 社内 CA を追加できるよう `/docker/certs` を読み込む仕組みも継承しています。
...
* 依存更新やリリース前の最低限の手動検証として、以下を実行します。
  ```bash
